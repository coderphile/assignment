...
@router.get("/find/{address_id}", response_model=schemas.Address)
...
@router.delete("/delete/{address_id}", response_model=schemas.Address)
...
def get_addresses_within_distance(
    ...
    if not addresses:
        raise HTTPException(status_code=201, detail="No address found") 
    return addresses
